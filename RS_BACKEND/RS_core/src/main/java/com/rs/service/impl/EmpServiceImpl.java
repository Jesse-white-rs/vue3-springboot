...
import com.rs.constants.SystemConstants;
...
public class EmpServiceImpl extends ServiceImpl<EmpMapper, Emp> implements EmpService {
    ...
    for (Emp e : emps) {
      empRoleDTOs.add(
          new EmpRoleDeptDTO(
              ...
              SystemConstants.USER_ONLINE,null,null));
    }
  ...
  public ResultResponse createEmp(EmpRoleDeptDTO empRoleDeptDTO) {
    ...
    log.info("创建用户请求: {}", empRoleDeptDTO);
    ...
    // 插入数据
    if (empMapper.createEmp(emp) != SystemConstants.DB_OPERATION_SUCCESS) {
  ...
  public ResultResponse updateEmp(EmpRoleDeptDTO empRoleDeptDTO) {
    Emp emp = empRoleDeptDTO.getEmp();
    if (SystemConstants.PASSWORD_NO_CHANGE_FLAG.equals(emp.getePassword())) {
    ...
    // 更新员工
    if (empMapper.updateEmp(emp) == SystemConstants.DB_OPERATION_FAILED) {
  ...
  public ResultResponse deleteEmp(Integer id) {
    if (empMapper.deleteEmp(id) == SystemConstants.DB_OPERATION_FAILED) {
  ...
  public ResultResponse deleteEmps(List<Integer> ids) {
    if (empMapper.deleteEmps(ids) == SystemConstants.DB_OPERATION_FAILED) {
      throw new BizException(ExceptionEnum.INTERNAL_SERVER_ERROR, "删除用户失败");
    }
    return ResultResponse.success();
  }
    ...
    public ResultResponse login(EmpRoleDeptDTO erdd) {
        ...
        try {
            ...
            String jwt = JwtUtils.generateJwtFromJson(
                    ...
                    erdd.isRememberMe() ? SystemConstants.TOKEN_EXPIRE_TIME_ONE_WEEK : null
            );
  ...
  public ResultResponse register(Emp emp) {
    ...
    try {
      ...
      emp.setePassword(passwordEncoder.encode(emp.getePassword()));
      // 设置临时部门
      emp.seteDeptid(SystemConstants.DEFAULT_DEPT_ID);
      // 设置启用状态
      emp.seteIsenabled(SystemConstants.USER_ENABLED);
      // 设置时间
      emp.seteUpdatetime(timeUtil.getCurrentTimestamp());
      ...
      // 赋予初始角色 - 普通用户
      empRoleService.insertEmpRole(emp.getId(), SystemConstants.ROLE_USER_ID);
      // 赋予基本权限
      empMenuService.addEmpMenu(emp.getId(), Collections.singletonList(SystemConstants.MENU_BASIC_PERMISSION_ID));
  ...
  public ResultResponse loginByToken() {
    ...
    // 如果用户存在，返回最新的用户信息
    if (userDetails != null) {
      if (empMapper.getEmp(new Emp(userDetails.getUsername())).geteIsenabled() == SystemConstants.USER_DISABLED) {
      ...
      // 在redis中设置该id的在线状态
        ...
        QywxDepartment qywxDepartment = qywxDepartmentMapper.selectOne(new QueryWrapper<QywxDepartment>()
                .eq("id", qywxUser.getDepartmentId()));
      return ResultResponse.success(
          new EmpRoleDeptDTO(emp, role, dept, SystemConstants.USER_ONLINE,qywxUser,qywxDepartment
                  )
          );
    } else {
