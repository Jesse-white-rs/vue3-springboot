...
import com.rs.config.QywxConfig;
...
public class QywxService {
    ...
    public String getAccessToken() {
        String tokenUrl = String.format(qywxConfig.getUrls().getGetToken(), 
                                       qywxConfig.getCorpId(), qywxConfig.getSecret());
        Map<String, Object> resp = restTemplate.getForObject(tokenUrl, Map.class);
        if (resp != null && SystemConstants.QYWX_SUCCESS_CODE.equals(resp.get("errcode").toString())) {
    ...
    public void syncQywxData() {
        ...
        for (QywxDepartment dept : deptEntities) {
            String userUrl = String.format(SystemConstants.QYWX_USER_LIST_URL, token, dept.getId());
            Map<String, Object> userResp = restTemplate.getForObject(userUrl, Map.class);
            if (!SystemConstants.QYWX_SUCCESS_CODE.equals(String.valueOf(userResp.get("errcode")))) continue;
    ...
    public void regQywxData() {
        ...
        for (QywxUser qyUser : qyUsers) {
            ...
            System.out.println("已存在" + count + "个职员");

            // 如果不存在，插入新职员
            if (count == SystemConstants.DB_OPERATION_FAILED) {
                Emp emp = new Emp();
                ...
                emp.setePassword(passwordEncoder.encode(SystemConstants.DEFAULT_INITIAL_PASSWORD));
    ...
    private String generateRandomUsername() {
        ...
        for (int i = 0; i < SystemConstants.RANDOM_USERNAME_LENGTH; i++) {
    ...
    public String generateTokenForEmp(Emp emp) {
        try {
            ...
            String token =   JwtUtils.generateJwtFromJson(
                                JSON.toJSONString(loginUserDetail), SystemConstants.TOKEN_EXPIRE_TIME_ONE_WEEK);
            return token;
        } catch (Exception e) {
    ...
    public Map<String, Object> getLaunchCode(String operatorUserId, String targetUserId) {
        ...
        if (response != null && SystemConstants.QYWX_SUCCESS_CODE.equals(response.get("errcode").toString())) {
            return response;
        }
