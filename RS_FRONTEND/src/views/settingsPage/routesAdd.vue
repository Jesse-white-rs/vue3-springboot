          ...
          <div class="modal-content-wrapper">
            ...
            <div class="modal-content">
              <el-scrollbar height="420px">
                <el-form ref="formRef" :model="store.routeAddFormData" :rules="store.ROUTE_ADD_FORM_RULES" label-width="80px">
                  <el-form-item label="父路由" prop="parentrouteid">
                    <el-input v-model="store.routeAddFormData.parentrouteid" placeholder="父路由的id" />
                  </el-form-item>
                  <el-form-item label="别名" prop="alias">
                    <el-input v-model="store.routeAddFormData.alias" />
                  </el-form-item>
                  <el-form-item label="路由组件" prop="name">
                    <el-input v-model="store.routeAddFormData.name" />
                  </el-form-item>
                  <el-form-item label="路由地址" prop="value">
                    <el-input v-model="store.routeAddFormData.value" />
                  </el-form-item>
                  <el-form-item label="路由类型" prop="type">
                    <el-select v-model="store.routeAddFormData.type" placeholder="请选择" clearable popper-style="z-index: 3100">
                      <el-option v-for="(item, index) in store.ROUTE_TYPE_OPTIONS" :key="index" :value="item.value" :label="item.label" />
                    </el-select>
                  </el-form-item>
                  <el-form-item label="图标" prop="icon">
                    <el-input v-model="store.routeAddFormData.icon" />
                  </el-form-item>
                  <el-form-item label="描述" prop="discription">
                    <el-input v-model="store.routeAddFormData.discription" />
                  </el-form-item>
                  <el-form-item label="排序" prop="sort">
                    <el-input v-model="store.routeAddFormData.sort" placeholder="数字越小越靠前" />
                  </el-form-item>
                  <el-form-item label="状态" prop="state">
                    <el-select v-model="store.routeAddFormData.state" placeholder="请选择" clearable popper-style="z-index: 3100">
                      <el-option v-for="(item, index) in store.ROUTE_STATE_OPTIONS" :key="index" :value="item.value" :label="item.label" />
                    </el-select>
                  </el-form-item>
                  <el-form-item label="重定向" prop="redirect">
                    <el-input v-model="store.routeAddFormData.redirect" />
                  </el-form-item>
                </el-form>
              </el-scrollbar>
            </div>
            <div class="modal-footer">
              <div class="form-actions">
                ...
                <button type="button" class="ios-btn primary" @click="handleSubmit">确认添加</button>
              </div>
            </div>
          </div>
        </div>
